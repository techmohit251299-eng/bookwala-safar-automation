...
def safe_verify_call(verify_name, verify_prompt, max_tokens):
    """
    Run a verification API call safely: if the call errors, or the response
    comes back with no usable text block (which happens occasionally and is
    NOT worth losing the whole run over - the actual script is already
    generated by this point), print a warning and move on instead of
    ...
    try:
        ...
        print(f"  ⚠️  {verify_name} check failed ({e}) - skipping this check, continuing pipeline.")
        return None
...
SHORT response दो।"""
    ...
    verification_result = safe_verify_call("Hindi Script Quality", verify_prompt, max_tokens=800)
...
SHORT response।"""
    ...
    verification_result = safe_verify_call("Emotional Depth", verify_prompt, max_tokens=500)
    if verification_result:
        print(f"  Result: {verification_result[:300]}...")
